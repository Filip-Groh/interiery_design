...
const DisplayImage = ({src, alt, width, height, classes="max-w-sm rounded-lg", isScaling=true}) => {
    return (
        <>
            <Image className={`${classes} shadow-2xl cursor-pointer ${isScaling? "hover:scale-[110%] sm:hover:scale-[150%] transition-transform" : ""}`} onClick={()=>document.getElementById(src).showModal()} width={width} height={height} src={src} alt={alt} />
