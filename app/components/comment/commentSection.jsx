        ...
        <>
            ...
            <ul className="flex flex-col gap-2 mb-4">
                {comments?.map((comment, index) => {
                    return <Comment key={comment.id} session={session} id={comment.id} comments={comments} setComments={setComments} name={comment.user.name} email={comment.user.email} image={comment.user.image} text={comment.text} likes={comment.likes} state={state[index]} />
                })}
            </ul>
        </>
    )
